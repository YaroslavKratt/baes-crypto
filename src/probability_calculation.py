...
class ProbabilityCalculationService:
  ...
  # P(C)
  @staticmethod
  def calculate_cypher_text_probabilities(open_texts_probabilities,
      keys_probabilities, cypher_table):
  ...
  def calculate_open_text_encrypted_to_cypher_text_probabilities(
    ...
    cypher_text_to_open_texts_and_keys = ProbabilityCalculationService.get_cypher_text_to_open_texts_and_keys(
      cypher_table)
    ...
    for open_text_index in range(len(open_texts_probabilities)):
      for cypher_text, key_open_text_pairs in cypher_text_to_open_texts_and_keys.items():
        probability = sum(
          keys_probabilities[key_index] * open_texts_probabilities[
            open_text_index]
          for key_index, open_text_idx in key_open_text_pairs
          if open_text_idx == open_text_index
        )
        open_text_encrypted_to_cypher_text_probabilities[
          (open_text_index, cypher_text)] = probability
  ...
  def calculate_open_text_dependent_on_cypher_text_probability(
    ...
    for (open_text,
         cypher_text), m_to_c_probability in open_text_encrypted_to_cypher_text_probabilities.items():
      probability = m_to_c_probability / cypher_text_probabilities.get(
        cypher_text)
      open_text_dependent_on_cypher_text_probabilities[
        (open_text, cypher_text)] = probability
